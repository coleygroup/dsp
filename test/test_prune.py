...
@pytest.fixture(params=[1.0, 0.1, 0.0])
def prob(request):
...
def test_retain_all(Y_mean, Y_var_tiny):
    threshold = torch.topk(Y_mean, 1, dim=0, sorted=True)[0][-1]
    idxs, _ = pruned_idxs_prob(
        Y_mean, Y_var_tiny, threshold, 0.0, torch.zeros(len(Y_mean), dtype=bool)
    )
...
def test_retain_none(Y_mean, Y_var_tiny):
    threshold = torch.topk(Y_mean, 1, dim=0, sorted=True)[0][-1]
    idxs, _ = pruned_idxs_prob(
        Y_mean, Y_var_tiny, threshold, 1.01, torch.zeros(len(Y_mean), dtype=bool)
    )

    torch.testing.assert_close(torch.arange(len(Y_mean)), idxs, rtol=0, atol=0)


@pytest.mark.parametrize("threshold,expected_prob", [(-1, 1), (2, 0)])
